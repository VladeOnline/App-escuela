...
class _TeacherDashboardPageState extends State<TeacherDashboardPage> {
  ...
  late final _studentsNotifier = StudentsNotifier(
  ...
  @override
  void dispose() { _studentsNotifier.dispose(); super.dispose(); }
  ...
  Widget _buildContent(int index) => switch (index) {
    0 => _DashboardView(notifier: _studentsNotifier),
    1 => ModulesPage(moduleType: ModuleType.reading, repository: _moduleRepository),
    2 => ModulesPage(moduleType: ModuleType.writing, repository: _moduleRepository),
      ...
      body: Row(
        children: [
          ...
          Expanded(
            child: Column(
              children: [
                TeacherTopbar(teacherName: _teacherName, onLogout: _onLogout),
...
class _StudentsCardState extends State<_StudentsCard> {
  ...
  void _filterGrade(int? g) { setState(() => _grade = g); widget.notifier.search(name: _searchCtrl.text, grade: g); }
  void _clear() { _searchCtrl.clear(); setState(() => _grade = null); widget.notifier.clearFilters(); }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        // Filtro
        Container(
          decoration: BoxDecoration(color: AppColors.surfaceCard, borderRadius: const BorderRadius.all(AppRadius.large), border: Border.all(color: AppColors.border)),
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Row(
                        ...
                        children: [
                          const Padding(padding: EdgeInsets.symmetric(horizontal: AppSpacing.sm), child: Icon(Icons.search_rounded, size: 20, color: AppColors.textHint)),
                          Container(width: 1, height: 25, color: AppColors.border),
                        ],
                      ),
                      suffixIcon: hasFilters ? IconButton(icon: const Icon(Icons.close_rounded, size: 18), onPressed: _clear) : null,
                ...
                MouseRegion(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: Container(
                      ...
                      decoration: BoxDecoration(border: Border.all(color: AppColors.border, width: 1.5), borderRadius: const BorderRadius.all(AppRadius.medium), color: AppColors.surfaceCard),
        ...
        Container(
          ...
          child: state.isLoading
              ? const Padding(padding: EdgeInsets.all(AppSpacing.xl), child: Center(child: CircularProgressIndicator(color: AppColors.primary)))
              : state.isEmpty
        ...
        child: Column(
          ...
          children: [
            Expanded(
              child: Stack(
                children: [
                  Container(width: double.infinity, color: gradeColor.withOpacity(0.07),
